...
socket.on('chat', (data) => {
    ...
    feedback.innerHTML = '';
    document.querySelector('.msg-disp-1').scrollTop = document.querySelector('.msg-disp-1').scrollHeight;
})
...
socket.on('online-users', (data) =>{
    users.innerHTML = ''
    data.forEach(user => {
      users.innerHTML += '<div class="name-contain">' + '<div class="online-circle"><span>Online</span></div>' + `<p class="online-name">${user}</p>` + '</div>'
    });
})